...
export function Room() {
    ...
    /*
    useEffect(() => { // redirecionar user se sala for fechada
        const roomRef = database.ref(`rooms/${roomId}`);
        roomRef.get().then(room => {
            if(room.val().endedAt) {
                alert("Room closed.");
                history.push('/') //redireciona pro home dps de sala fechada
            }
        })
    }, [history, roomId])
    */
    ...
    async function handleLikeQuestion(questionId:string, likeId: string | undefined) {
        if (likeId) { //add-remove like: um usuario n pode dar mais de um like no msm item
